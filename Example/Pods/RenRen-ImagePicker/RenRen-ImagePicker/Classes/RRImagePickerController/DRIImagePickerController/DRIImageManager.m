...
@interface DRIImageManager ()
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
@end

@implementation DRIImageManager

CGSize DRIAssetGridThumbnailSize;
CGFloat DRIScreenWidth;
CGFloat DRIScreenScale;

static DRIImageManager *manager;
...
- (void)setPhotoWidth:(CGFloat)photoWidth {
    ...
    DRIScreenWidth = photoWidth / 2;
}

- (void)setColumnNumber:(NSInteger)columnNumber {
    [self configDRIScreenWidth];

    _columnNumber = columnNumber;
    CGFloat margin = 4;
    CGFloat itemWH = (DRIScreenWidth - 2 * margin - 4) / columnNumber - margin;
    DRIAssetGridThumbnailSize = CGSizeMake(itemWH * DRIScreenScale, itemWH * DRIScreenScale);
}

- (void)configDRIScreenWidth {
    DRIScreenWidth = [UIScreen mainScreen].bounds.size.width;
    // 测试发现，如果scale在plus真机上取到3.0，内存会增大特别多。故这里写死成2.0
    DRIScreenScale = 2.0;
    if (DRIScreenWidth > 700) {
        DRIScreenScale = 1.5;
    }
}
...
/// Get Album 获得相册/相册数组
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(DRIAlbumModel *model))completion {
    __block DRIAlbumModel *model;
...
- (void)getAllAlbums:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(NSArray<DRIAlbumModel *> *))completion{
...
/// Get Assets 获得照片数组
- (void)getAssetsFromFetchResult:(PHFetchResult *)result cameraRoll:(BOOL)isCameraRoll completion:(void (^)(NSArray<DRIAssetModel *> *))completion {
    DRIImagePickerConfig *config = [DRIImagePickerConfig sharedInstance];
    return [self getAssetsFromFetchResult:result allowPickingVideo:config.allowPickingVideo allowPickingImage:config.allowPickingImage cameraRoll:isCameraRoll completion:completion];
}

- (void)getAssetsFromFetchResult:(PHFetchResult *)result allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage cameraRoll:(BOOL)isCameraRoll completion:(void (^)(NSArray<DRIAssetModel *> *))completion {
    ...
    [result enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(PHAsset *asset, NSUInteger idx, BOOL * _Nonnull stop) {
        DRIAssetModel *model = [self assetModelWithAsset:asset allowPickingVideo:allowPickingVideo allowPickingImage:allowPickingImage];
        if (model) {
            if (isCameraRoll) {
...
- (void)getAssetFromFetchResult:(PHFetchResult *)result atIndex:(NSInteger)index allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(DRIAssetModel *))completion {
    PHAsset *asset;
    @try {
        asset = result[index];
    }
...
- (DRIAssetModel *)assetModelWithAsset:(PHAsset *)asset allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage {
    BOOL canSelect = YES;
    ...
    DRIAssetModel *model;
    DRIAssetModelMediaType type = [self getAssetType:asset];
    if (!allowPickingVideo && type == DRIAssetModelMediaTypeVideo) return nil;
    if (!allowPickingImage && type == DRIAssetModelMediaTypePhoto) return nil;
    if (!allowPickingImage && type == DRIAssetModelMediaTypePhotoGif) return nil;
    ...
    NSString *timeLength = type == DRIAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",phAsset.duration] : @"";
    timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
    model = [DRIAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}

- (DRIAssetModelMediaType)getAssetType:(PHAsset *)asset {
    DRIAssetModelMediaType type = DRIAssetModelMediaTypePhoto;
    PHAsset *phAsset = (PHAsset *)asset;
    if (phAsset.mediaType == PHAssetMediaTypeVideo)      type = DRIAssetModelMediaTypeVideo;
    else if (phAsset.mediaType == PHAssetMediaTypeAudio) type = DRIAssetModelMediaTypeAudio;
    else if (phAsset.mediaType == PHAssetMediaTypeImage) {
        if (@available(iOS 9.1, *)) {
            // if (asset.mediaSubtypes == PHAssetMediaSubtypePhotoLive) type = DRIAssetModelMediaTypeLivePhoto;
        }
        // Gif
        if ([[phAsset valueForKey:@"filename"] hasSuffix:@"GIF"]) {
            type = DRIAssetModelMediaTypePhotoGif;
        }
    }
    return type;
}
...
- (void)getPhotosBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *totalBytes))completion {
    ...
    for (NSInteger i = 0; i < photos.count; i++) {
        ...
        [[PHImageManager defaultManager] requestImageDataForAsset:model.asset options:options resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
            if (model.type != DRIAssetModelMediaTypeVideo) dataLength += imageData.length;
            assetCount ++;
...
- (PHImageRequestID)getPhotoWithAsset:(PHAsset *)asset completion:(void (^)(UIImage *, NSDictionary *, BOOL isDegraded))completion {
    CGFloat fullScreenWidth = DRIScreenWidth;
...
- (PHImageRequestID)getPhotoWithAsset:(PHAsset *)asset photoWidth:(CGFloat)photoWidth completion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded))completion progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler networkAccessAllowed:(BOOL)networkAccessAllowed {
    CGSize imageSize;
    if (photoWidth < DRIScreenWidth && photoWidth < _photoPreviewMaxWidth) {
        imageSize = DRIAssetGridThumbnailSize;
    } else {
        PHAsset *phAsset = (PHAsset *)asset;
        CGFloat aspectRatio = phAsset.pixelWidth / (CGFloat)phAsset.pixelHeight;
        CGFloat pixelWidth = photoWidth * DRIScreenScale;
        // 超宽图片
        if (aspectRatio > 1.8) {
...
/// Get postImage / 获取封面图
- (PHImageRequestID)getPostImageWithAlbumModel:(DRIAlbumModel *)model completion:(void (^)(UIImage *))completion {
    id asset = [model.result lastObject];
    if (!self.sortAscendingByModificationDate) {
        asset = [model.result firstObject];
    }
    return [[DRIImageManager manager] getPhotoWithAsset:asset photoWidth:80 completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (PHImageRequestID)getOriginalPhotoWithAsset:(PHAsset *)asset progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler newCompletion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded))completion {
    ...
    option.resizeMode = PHImageRequestOptionsResizeModeFast;
    return [[PHImageManager defaultManager] requestImageForAsset:asset targetSize:CGSizeMake(asset.pixelWidth, asset.pixelHeight) contentMode:PHImageContentModeAspectFit options:option resultHandler:^(UIImage *result, NSDictionary *info) {
        BOOL downloadFinined = (![[info objectForKey:PHImageCancelledKey] boolValue] && ![info objectForKey:PHImageErrorKey]);
        if (downloadFinined && result) {
            result = [self fixOrientation:result];
...
- (void)startExportVideoWithVideoAsset:(AVURLAsset *)videoAsset presetName:(NSString *)presetName success:(void (^)(NSString *outputPath))success failure:(void (^)(NSString *errorMessage, NSError *error))failure {
    ...
    if ([presets containsObject:presetName]) {
        ...
        if ([DRIImagePickerConfig sharedInstance].needFixComposition) {
...
- (DRIAlbumModel *)modelWithResult:(PHFetchResult *)result name:(NSString *)name isCameraRoll:(BOOL)isCameraRoll needFetchAssets:(BOOL)needFetchAssets {
    DRIAlbumModel *model = [[DRIAlbumModel alloc] init];
    model.name = name;
...
- (DRIAssetModel *)createModelWithAsset:(PHAsset *)asset {
    DRIAssetModelMediaType type = [[DRIImageManager manager] getAssetType:asset];
    NSString *timeLength = type == DRIAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",asset.duration] : @"";
    timeLength = [[DRIImageManager manager] getNewTimeFromDurationSecond:timeLength.integerValue];
    DRIAssetModel *model = [DRIAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}
...
- (PHImageRequestID)NewGetPhotoWithAsset:(PHAsset *)asset completion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded))completion progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler{
    __block UIImage *image;
    PHImageRequestOptions *option = [[PHImageRequestOptions alloc] init];
    option.resizeMode = PHImageRequestOptionsResizeModeFast;
    int32_t imageRequestID = [[PHImageManager defaultManager] requestImageForAsset:asset targetSize:PHImageManagerMaximumSize contentMode:PHImageContentModeAspectFill options:option resultHandler:^(UIImage *result, NSDictionary *info) {
        __block BOOL degraded = [[info objectForKey:PHImageResultIsDegradedKey] boolValue];
        BOOL downloadFinined = (![[info objectForKey:PHImageCancelledKey] boolValue] && ![info objectForKey:PHImageErrorKey]);
        if (downloadFinined && result) {
            result = [self fixOrientation:result];
            image = result;
        ...
        [[PHImageManager defaultManager] requestImageDataForAsset:asset options:options resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
            ...
            resultImage = [self fixOrientation:resultImage];
            resultImage.originData = imageData;
            if (completion) completion(resultImage,info,degraded);
        }];
    }];
    return imageRequestID;
}
...
- (BOOL)isLongImage:(UIImage *)image{
    return (image.size.width / image.size.height) <= 0.37;
}
...
//@implementation DRISortDescriptor
//
//- (id)reversedSortDescriptor {
//    return [NSNumber numberWithBool:![DRIImageManager manager].sortAscendingByModificationDate];
//}
//
//@end
