...
/*
 经过测试，比起xib的方式，把DRIAssetCell改用纯代码的方式来写，滑动帧数明显提高了（约提高10帧左右）
...
@class DRIAlbumCell, DRIAssetCell;
@protocol DRIImagePickerControllerDelegate;
@interface DRIImagePickerController : UINavigationController <UINavigationControllerDelegate>
...
@property (nonatomic, strong) NSMutableArray<DRIAssetModel *> *selectedModels;
...
@property (nonatomic, assign) BOOL onlyTakeVideo; ///< 需要圆形裁剪框
...
#pragma mark -
@property (nonatomic, copy) NSString *photoSelPreviewImageName __attribute__((deprecated("Use -photoSelPreviewImage.")));
...
@property (nonatomic, strong) UIImage *photoSelPreviewImage;
#pragma mark -
...
// For method annotations, see the corresponding method in DRIImagePickerControllerDelegate / 方法注释见DRIImagePickerControllerDelegate中对应方法
...
@property (nonatomic, weak) id<DRIImagePickerControllerDelegate> pickerDelegate;

@end


@protocol DRIImagePickerControllerDelegate <NSObject>
@optional
// The picker should dismiss itself; when it dismissed these callback will be called.
// You can also set autoDismiss to NO, then the picker don't dismiss itself.
// If isOriginalPhoto is YES, user picked the original photo.
// You can get original photo with asset, by the method [[DRIImageManager manager] getOriginalPhotoWithAsset:completion:].
// The UIImage Object in photos default width is 828px, you can set it by photoWidth property.
// 这个照片选择器会自己dismiss，当选择器dismiss的时候，会执行下面的代理方法
// 你也可以设置autoDismiss属性为NO，选择器就不会自己dismis了
// 如果isSelectOriginalPhoto为YES，表明用户选择了原图
// 你可以通过一个asset获得原图，通过这个方法：[[DRIImageManager manager] getOriginalPhotoWithAsset:completion:]
// photos数组里的UIImage对象，默认是828像素宽，你可以通过设置photoWidth属性的值来改变它
- (void)imagePickerController:(DRIImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto;
...
@interface DRIAlbumPickerController : UIViewController
...
@interface DRICommonTools : NSObject
+ (BOOL)dri_isIPhoneX;
...
@interface DRIImagePickerConfig : NSObject
...
@property (nonatomic, copy) void (^gifImagePlayBlock)(DRIPhotoPreviewView *view, UIImageView *imageView, NSData *gifData, NSDictionary *info);
@end
