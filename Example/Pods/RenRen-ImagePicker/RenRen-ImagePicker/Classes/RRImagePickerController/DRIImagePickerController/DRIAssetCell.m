...
@implementation DRIAssetCell

- (instancetype)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reload:) name:@"DRI_PHOTO_PICKER_RELOAD_NOTIFICATION" object:nil];
    return self;
}
...
- (void)setModel:(DRIAssetModel *)model {
    ...
    int32_t imageRequestID = [[DRIImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
    ...
    self.indexLabel.hidden = !(self.selectPhotoButton.isSelected && self.model.type <= DRIAssetCellTypePhotoGif);
...
- (void)setShowSelectBtn:(BOOL)showSelectBtn {
    ...
    BOOL selectable = [[DRIImageManager manager] isPhotoSelectableWithAsset:self.model.asset];
...
- (void)setType:(DRIAssetCellType)type {
    _type = type;
    if (type == DRIAssetCellTypePhoto || type == DRIAssetCellTypeLivePhoto || (type == DRIAssetCellTypePhotoGif && self.allowPickingGif) || self.allowPickingMultipleVideo) {
    ...
    } else if (type == DRIAssetCellTypePhotoGif && self.allowPickingGif) {
        self.bottomView.hidden = NO;
...
- (void)selectPhotoButtonClick:(UIButton *)sender {
    ...
    if (sender.isSelected) {
        [UIView showOscillatoryAnimationWithLayer:_selectImageView.layer type:RHCOscillatoryAnimationToBigger];
        // 用户选中了该图片，提前获取一下大图
        [self requestBigImage];
    } else { // 取消选中，取消大图的获取
        [self cancelBigImageRequest];
...
- (void)requestBigImage {
    ...
    _bigImageRequestID = [[DRIImageManager manager] requestImageDataForAsset:_model.asset completion:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
        [self hideProgressView];
...
- (void)reload:(NSNotification *)noti {
    DRIImagePickerController *driImagePickVc = (DRIImagePickerController *)noti.object;
    if (self.model.isSelected && driImagePickVc.showSelectedIndex) {
        self.index = [driImagePickVc.selectedAssetIds indexOfObject:self.model.asset.localIdentifier] + 1;
    }
    self.indexLabel.hidden = !(self.selectPhotoButton.isSelected && self.model.type <= DRIAssetCellTypePhotoGif);
    if (driImagePickVc.selectedModels.count >= driImagePickVc.maxImagesCount && driImagePickVc.showPhotoCannotSelectLayer && !self.model.isSelected) {
        self.cannotSelectLayerButton.backgroundColor = driImagePickVc.cannotSelectLayerColor;
        self.cannotSelectLayerButton.hidden = NO;
    } else {
        if (driImagePickVc.selectedModels.count &&
            self.model.isImage != driImagePickVc.selectedModels.lastObject.isImage) {
            self.cannotSelectLayerButton.backgroundColor = driImagePickVc.cannotSelectLayerColor;
            self.cannotSelectLayerButton.hidden = NO;
        }else{
...
- (UIImageView *)videoImgView {
    if (_videoImgView == nil) {
        ...
        [videoImgView setImage:[UIImage dri_imageNamedFromMyBundle:@"VideoSendIcon"]];
...
- (void)layoutSubviews {
    ...
    if (self.allowPreview) {
        self.selectPhotoButton.frame = CGRectMake(self.width - 44, 0, 44, 44);
    } else {
        self.selectPhotoButton.frame = self.bounds;
    }
    self.selectImageView.frame = CGRectMake(self.width - 29, 3, 26, 26);
//    if (_selectImageView.image.size.width <= 26) {
//        _selectImageView.contentMode = UIViewContentModeCenter;
//    } else {
        _selectImageView.contentMode = UIViewContentModeScaleToFill;
//    }
    self.indexLabel.frame = _selectImageView.frame;
    self.imageView.frame = CGRectMake(0, 0, self.width, self.height);
    
    CGFloat progressWH = 20;
    CGFloat progressXY = (self.width - progressWH) / 2;
    self.progressView.frame = CGRectMake(progressXY, progressXY, progressWH, progressWH);

    self.bottomView.frame = CGRectMake(0, self.height - 20, self.width, 20);
    self.videoImgView.frame = CGRectMake(8, 6, 13.5, 8);
    self.timeLength.frame = CGRectMake(self.videoImgView.right, 0, self.width - self.videoImgView.right - 5, 20);
...
@interface DRIAlbumCell ()
@property (weak, nonatomic) UIImageView *posterImageView;
@property (weak, nonatomic) UILabel *titleLabel;
@end

@implementation DRIAlbumCell
...
- (void)setModel:(DRIAlbumModel *)model {
    _model = model;
...
- (void)layoutSubviews {
    [super layoutSubviews];
    _selectedCountButton.frame = CGRectMake(self.contentView.width - 24, 23, 24, 24);
...
@implementation DRIAssetCameraCell
