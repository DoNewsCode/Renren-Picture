...
@implementation DRIImageCropManager
...
+ (UIImage *)cropImageView:(UIImageView *)imageView toRect:(CGRect)rect zoomScale:(double)zoomScale containerView:(UIView *)containerView {
    ...
    CGFloat xMargin = containerView.width - CGRectGetMaxX(rect) - rect.origin.x;
    ...
    cropedImage = [[DRIImageManager manager] fixOrientation:cropedImage];
...
+ (UIImage *)sd_dri_animatedGIFWithData:(NSData *)data {
    ...
    if (count <= 1) {
        ...
        NSInteger maxCount = [DRIImagePickerConfig sharedInstance].gifPreviewMaxImagesCount ?: 50;
